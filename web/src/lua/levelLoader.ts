 ...
 */
function getImageManifest(): Promise<Set<string>> {
...
export async function loadLevelModels(levelName: string): Promise<LevelData> {
  ...
  try {
    ...
    // Real lookup (docs/009) - imgList() in level_creation.lua uses this to
    // discover every numbered frame ("_00", "_01", ...) of an animation, not
    // just the first. filename arrives in the same "images/<...>" form the
    // manifest keys are in, so this is a direct membership check.
    lua.global.set("file_exists", (filename: string) => imageManifest.has(filename));
